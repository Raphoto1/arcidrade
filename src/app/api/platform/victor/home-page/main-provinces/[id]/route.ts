...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await withPrismaRetry(() =>
      prisma.homePageDataMainProvinces.update({
        where: { id: numId },
        data: {
          title: title.trim(),
    ...
    console.error("Error al actualizar provincia principal:", error);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await withPrismaRetry(() => prisma.homePageDataMainProvinces.delete({ where: { id: numId } }));
    ...
    console.error("Error al eliminar provincia principal:", error);
    return NextResponse.json({ error: "Error interno del servidor" }, { status: 500 });
  }
}
