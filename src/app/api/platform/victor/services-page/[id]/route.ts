...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error?.code === "P2025") {
      return NextResponse.json({ error: "Card de Services no encontrada" }, { status: 404 });
    }
    console.error("Error al actualizar card de Services:", error);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await withPrismaRetry(() => servicesPageData.delete({ where: { id: numId } }));
    ...
    console.error("Error al eliminar card de Services:", error);
    return NextResponse.json({ error: "Error interno del servidor" }, { status: 500 });
  }
}
