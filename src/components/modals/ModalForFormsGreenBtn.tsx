...
export default function ModalForFormsGreenBtn({ children, title, btnClassName }: ModalProps) {
  ...
  return (
    <div>
      {title && (
        <button className={`btn bg-success h-auto p-1 w-full text-white ${btnClassName || ""}`} onClick={openModal}>
          {title}
        </button>
      )}
