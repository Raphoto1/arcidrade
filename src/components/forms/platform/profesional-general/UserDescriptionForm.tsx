...
export default function UserDescriptionFormGeneral() {
  const { closeModal } = useModal();
  const { data, mutate } = useProfesionalGeneral();
  const {
    register,
    handleSubmit,
  } = useForm({
  ...
  const onSubmit = handleSubmit(async (formData) => {
    const response = await useHandleSubmitText(formData, "/api/platform/profesional/description");
    if (response.ok) {
      mutate();
      closeModal();
    }
