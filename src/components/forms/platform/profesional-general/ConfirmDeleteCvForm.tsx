...
export default function ConfirmDeleteCvFormGeneral() {
  const { mutate } = useProfesionalGeneral();
  const { closeModal } = useModal();

  const handleDelete = async () => {
    const response = await fetch("/api/platform/upload/cv", {
      method: "DELETE",
    });
    if (!response.ok) {
  ...
  return (
    <div className='flex flex-col justify-center align-middle items-center'>
      <h1 className='text-2xl fontArci text-center pb-5'>Se Eliminara el CV de Manera Permanente</h1>
