...
import { useInstitutionCertification, useInstitutionCertifications, useInstitutionGoal, useInstitutionGoals } from "@/hooks/usePlatInst";

export default function InstitutionGoalUpdateForm(props: any) {
  const { closeModal } = useModal();
  const { mutate } = useInstitutionGoals();
  const { data } = useInstitutionGoal(props.id);
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    const response = await useHandleSubmitText(payload, `/api/platform/institution/goal/${props.id}`);
    if (response.ok) {
      mutate();
      closeModal();
    }
