...
import { useInstitutionCertification, useInstitutionCertifications } from "@/hooks/usePlatInst";

export default function InstitutionCertificationUpdateForm(props: any) {
  const { closeModal } = useModal();
  const { mutate } = useInstitutionCertifications();
  const { data } = useInstitutionCertification(props.id);
  ...
  // Cargar valores previos cuando data esté disponible
  useEffect(() => {
    if (data?.payload) {
      reset({
        title: data.payload.title || "",
        institution: data.payload.institution || "",
        year: data.payload.year ? new Date(data.payload.year).toISOString().split('T')[0] : "",
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    const response = await useHandleSubmitText(payload, `/api/platform/institution/certification/${props.id}`);
    if (response.ok) {
      mutate();
      closeModal();
    }
      ...
      <div className='flex justify-center items-center h-1/2 p-2 min-w-xl md:min-w-xl'>
        <div className='flex-col justify-start h-full bg-gray-200 w-2/3 align-middle items-center rounded-sm p-4 md:justify-center'>
          ...
          <form onSubmit={onSubmit} className='form justify-center align-middle pl-2 min-w-full grid gap-4 mt-4'>
            ...
            <div>
              <label htmlFor='institution' className='block font-semibold mb-1'>
                Institución otorgante
              </label>
              <input type='text' {...register("institution", { required: "Este campo es obligatorio" })} className='input input-bordered w-full' />
              {errors.institution?.message && <span className='text-xs text-red-500'>{String(errors.institution.message)}</span>}
            </div>
