...
export default function FileExperienceForm(id: any) {
  ...
  const path = `/api/platform/upload/experience/${id.id}`
  ...
  const onSubmit = handleSubmit(async (data: any) => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(result?.error || "No se pudo guardar el respaldo de experiencia", "error");
        return;
      }
      showToast("Respaldo de experiencia guardado correctamente", "success");
      mutate();
    ...
    <div className='flex w-full justify-center items-center'>
      <div className='flex justify-center items-center h-1/2 p-2 min-w-xl md:min-w-xl'>
        <div className='flex-col justify-start h-full bg-gray-200 w-2/3 align-middle items-center rounded-sm p-4 md:justify-center'>
          <h2 className='text-2xl font-bold test-start font-var(--font-oswald)'>Respaldo Experiencia</h2>
