...
import { usePausedProcesses } from "@/hooks/useProcess";

export default function PausedProcess() {
  const { data, error, isLoading, mutate } = usePausedProcesses();
    ...
    <div className='grid grid-cols-1 w-full p-4 md:max-h-3/4 md:max-w-3/4 md:justify-center md:align-middle md:items-center'>
      <div className='collapse collapse-arrow bg-gray-300 rounded-t-md mb-2'>
        ...
        <div className='collapse-title flex items-center justify-between'>
          <h2 className='text-2xl fontArci text-center w-full'>Procesos Pausados</h2>
          <FiChevronDown className='w-6 h-6 text-gray-700 ml-2' />
        </div>
        <div className='collapse-content'>
          <h3 className='text-xl fontArci text-center w-full'>Contacta a Arcidrade para más información</h3>
          <div className='processss flex flex-col justify-center align-middle bg-gray-100 p-1 gap-2'>
            {data && data.payload.length > 0 ? (
              ...
              <p className='text-center'>No hay procesos pausados</p>
            )}
          </div>
        </div>
      </div>
