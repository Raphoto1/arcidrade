...
export default function PausedInstitutions() {
  ...
  return (
    <div className='flex-col justify-start bg-gray-200 w-full align-middle items-center rounded-sm p-2 md:justify-center md:gap-4 md:h-auto'>
      <div className='pb-2'>
        <h1 className='text-2xl fontArci text-center'>Instituciones Pausadas (en desarrollo)</h1>
      </div>
      ...
      <div>
        {data?.payload.length === 0 && !isLoading && <p className="text-center">No hay instituciones pausadas.</p>}
        {isLoading && <p className="text-center">Cargando instituciones pausadas...</p>}
        {error && <p className="text-center text-red-500">Error al cargar las instituciones pausadas.</p>}
        {data?.payload.map((institution: any) => (
          <InstitutionPill key={institution.id} institution={institution.referCode} />
        ))}
      </div>
    </div>
  );
