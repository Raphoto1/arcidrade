    ...
    <li>
      <button className={`text-left p-3 hover:bg-base-200 rounded-lg w-full ${isSelected ? "bg-base-200" : ""}`} onClick={() => onSelect(process)}>
        <div className='flex flex-col'>
          ...
          <span className='text-xs text-gray-500'>Estado: {process.status || "Archivado"}</span>
        </div>
      </button>
    </li>
  );
...
export default function ArchivedProcess() {
  ...
  const validProcesses = archivedProcesses.filter((process: any) => process && process.id);
    ...
    <div className='flex justify-center'>
      <div className='grid grid-cols-1 gap-4 mt-4 pt-0 md:max-h-3/4 md:max-w-full md:justify-center md:align-middle md:items-center md:w-4/5 bg-gray-100'>
        ...
        <div className='flex md:justify-around flex-col md:flex-row bg-gray-300 rounded-t-md p-2'>
          <span></span>
          <h2 className='text-2xl fontArci text-center bg-gray-300 rounded-t-md'>Procesos Archivados ({filteredProcesses.length})</h2>
        ...
        {isLoading && (
          <div className='flex justify-center items-center p-8'>
            ...
            <span className='ml-2'>Cargando procesos archivados...</span>
          </div>
        )}

        {error && (
          <div className='alert alert-error max-w-md mx-auto'>
            <p>Error al cargar los procesos archivados</p>
          </div>
        )}
            ...
            {validProcesses.length > 0 ? (
              ...
              <div className='text-center p-12'>
                <div className='flex flex-col items-center gap-4'>
                  ...
                  <p className='text-gray-500 text-xl'>No hay procesos archivados</p>
                  <p className='text-gray-400 text-sm'>Los procesos archivados aparecerán aquí</p>
                </div>
              </div>
            )}
          </div>
