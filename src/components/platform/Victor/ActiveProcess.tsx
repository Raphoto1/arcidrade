...
function ProcessDropdownItem({ process, onSelect, isSelected }: { 
  process: any, 
  ...
  return (
    <li>
      <button 
        className={`text-left p-3 hover:bg-base-200 rounded-lg w-full ${isSelected ? 'bg-base-200' : ''}`}
        onClick={() => onSelect(process)}
      >
        <div className="flex flex-col">
          <span className="font-semibold text-[var(--main-arci)]">
            {process.position}
          </span>
          <span className="text-sm font-medium text-gray-700">
            Cliente: {institutionData?.name || 'Cargando cliente...'}
          </span>
          <span className="text-sm text-gray-600">
            Especialidad: {process.main_speciality}
          </span>
          <span className="text-xs text-gray-500">
            Estado: {process.status || 'Activo'}
          </span>
        </div>
      </button>
    </li>
...
function SelectedProcessDisplay({ process }: { process: any }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <span className="text-sm text-gray-600">
        {institutionData?.name || 'Cargando cliente...'}
      </span>
    </div>
  );
}
    ...
    <div className='flex justify-center'>
      <div className='grid grid-cols-1 gap-4 mt-4 pt-0 md:max-h-3/4 md:max-w-full md:justify-center md:align-middle md:items-center md:w-4/5 bg-gray-100'>
        ...
        <div className='flex md:justify-around flex-col md:flex-row bg-gray-300 rounded-t-md p-2'>
          <span></span>
          <h2 className='text-2xl fontArci text-center bg-gray-300 rounded-t-md'>Procesos Activos</h2>
        ...
        {error && (
          <div className="alert alert-error max-w-md mx-auto">
            <p>Error al cargar los procesos activos</p>
          </div>
        )}
                ...
                <div ref={dropdownRef} className={`dropdown dropdown-bottom w-full relative z-50 ${isDropdownOpen ? 'dropdown-open' : ''}`}>
                  ...
                  >
                    <span className="text-left">
                      {selectedProcess 
                        ? <SelectedProcessDisplay process={selectedProcess} />
                        : 'Seleccionar proceso para ver detalles'
                      }
                    </span>
                    <svg 
                  ...
                  <ul tabIndex={0} className="dropdown-content z-[9999] menu p-2 shadow-2xl bg-base-100 rounded-box w-full max-h-60 overflow-auto border border-gray-200">
                  ...
                  <div ref={filterDropdownRef} className={`dropdown dropdown-bottom relative z-40 ${isFilterDropdownOpen ? 'dropdown-open' : ''}`}>
                    ...
                    >
                      <span className="text-left truncate">
                        {institutionFilter || 'Todas las instituciones'}
                      </span>
                      <svg 
                    ...
                    <ul tabIndex={0} className="dropdown-content z-[9998] menu p-2 shadow-xl bg-base-100 rounded-box w-full max-h-48 overflow-auto border border-gray-200">
                      <li>
                        <button 
                      ...
                      {availableInstitutions.map((institution) => (
                        <li key={institution}>
                          <button 
                            className={`text-left p-2 hover:bg-base-200 rounded-lg w-full ${institutionFilter === institution ? 'bg-base-200 font-semibold' : ''}`}
            ...
            {validProcesses.length > 0 ? (
              ...
              <div className="text-center p-12">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-gray-500 text-xl">No hay procesos activos</p>
                  <p className="text-gray-400 text-sm">Los procesos activos aparecerán aquí</p>
                </div>
              </div>
            )}
          </div>
      ...
      {uniqueUserIds.filter((userId: any) => !institutionsData[userId]).map((userId: any) => (
