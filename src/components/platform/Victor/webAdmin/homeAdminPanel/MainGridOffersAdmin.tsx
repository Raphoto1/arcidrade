...
const staticFallback = [
  { title: "Ofertas de empleo", description: "Tarjeta destacada" },
  { title: "Ofertas Profesionales", description: "Tarjeta destacada" },
  { title: "Especialidades Solicitadas", description: "Tarjeta destacada" },
  { title: "Regiones", description: "Tarjeta destacada" },
];

const fetcher = (url: string) => fetch(url).then((r) => r.json());
const MAX_MAIN_OFFERS = 4;

const emptyForm = { title: "", image: "", link: "", order: "" };
type MainOfferFormState = typeof emptyForm;

function validateMainOfferForm(value: MainOfferFormState) {
  ...
  if (value.link.trim()) {
    ...
    if (!linkValue.startsWith("/")) {
      try {
        ...
        if (!["http:", "https:"].includes(url.protocol)) {
          return "El enlace debe ser ruta interna (/offers) o URL http(s)";
        }
      } catch {
...
function MainOfferForm({
  ...
  sectionTitle?: string;
  value: MainOfferFormState;
  submitLabel: string;
  submitting: boolean;
  submitDisabled?: boolean;
  error: string;
  onChange: (next: MainOfferFormState) => void;
  onSubmit: () => void;
  onCancel?: () => void;
}) {
  return (
      ...
      <div className='grid gap-2 md:grid-cols-2'>
        <label className='form-control'>
          ...
          <input
            ...
            placeholder='Ej: Ofertas de empleo'
            maxLength={200}
          />
        </label>
        <label className='form-control'>
      ...
      <label className='form-control'>
        <span className='label-text flex items-center gap-1 text-xs text-gray-500'>
          ...
          <span
            ...
            data-tip='Acepta rutas internas como /offers o /about, y tambien URLs completas con http(s). Si lo dejas vacio, se guarda /offers.'>
        ...
        <input
          ...
          placeholder='/offers (por defecto)'
        />
      </label>

      {value.image.trim() && !validateMainOfferForm({ ...value, title: "ok", order: "", link: "" }) && (
        <div className='overflow-hidden rounded-xl border border-gray-200 bg-white'>
          <div className='relative h-32 w-full'>
            ...
            <p className='absolute bottom-2 left-2 right-2 text-sm font-semibold text-white drop-shadow-lg'>
              {value.title.trim() || "Titulo de tarjeta"}
            </p>
          </div>
        </div>
      )}
...
function MainOffersList({
  ...
  onStartEdit: (item: MainOfferItem) => void;
...
export default function MainGridOffersAdmin() {
  const { data, isLoading, mutate } = useSWR<{ success: boolean; payload: MainOfferItem[] }>(
    "/api/platform/victor/home-page/main-offers",
    fetcher
  );

  const items: MainOfferItem[] = useMemo(() => {
    return data?.success ? (data.payload ?? []) : [];
  }, [data?.success, data?.payload]);
  const hasDbData = Boolean(!isLoading && data?.success && items.length > 0);
  const canCreateMore = items.length < MAX_MAIN_OFFERS;

  const [orderedItems, setOrderedItems] = useState<MainOfferItem[]>([]);
  ...
  const handleCreate = async () => {
    if (!canCreateMore) {
      setCreateError(`Solo puedes tener ${MAX_MAIN_OFFERS} tarjetas principales.`);
      return;
    }

    const validationError = validateMainOfferForm(form);
    if (validationError) {
    ...
    setCreateError("");
    try {
      const res = await fetch("/api/platform/victor/home-page/main-offers", {
        method: "POST",
      ...
      if (!res.ok) {
        setCreateError(json.error ?? "Error al crear tarjeta");
        return;
      }
      setForm(emptyForm);
      mutate();
    } catch {
      setCreateError("Error al crear tarjeta");
    } finally {
      setCreating(false);
    }
  };

  const startEdit = (item: MainOfferItem) => {
    setEditId(item.id);
    setEditError("");
    setCreateError("");
    setEditForm({
  ...
  const handleSaveEdit = async () => {
    const validationError = validateMainOfferForm(editForm);
    if (validationError || editId === null) {
      setEditError(validationError || "No se pudo identificar la tarjeta");
      return;
    }
    ...
    setEditError("");
    try {
      const res = await fetch(`/api/platform/victor/home-page/main-offers/${editId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id);
    try {
      await fetch(`/api/platform/victor/home-page/main-offers/${id}`, { method: "DELETE" });
      mutate();
    } finally {
      setDeletingId(null);
    }
    ...
    try {
      const responses = await Promise.all(
        withRecalculatedOrder.map((item) =>
          fetch(`/api/platform/victor/home-page/main-offers/${item.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              title: item.title,
    ...
    <details className='group mt-4 rounded-2xl border border-gray-200 bg-gray-50'>
      <summary className='list-none cursor-pointer p-4'>
        <div className='flex items-start justify-between gap-3'>
          <div>
            <h3 className='font-semibold text-(--main-arci)'>Primer grid principal</h3>
            <p className='mt-1 text-xs text-gray-500'>Tarjetas destacadas del primer grid del home.</p>
          </div>
      ...
      <div className='px-4 pb-4'>
        <div className='rounded-2xl border border-gray-200 bg-white p-4'>
          <p className='mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400'>Orden de tarjetas</p>
        ...
        <div className='mt-4 rounded-2xl border border-gray-200 bg-white p-4'>
          <p className='mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400'>Formulario de tarjeta</p>
          {editId !== null && (
            <p className='mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700'>
              Editando tarjeta #{editId}. Guarda cambios o cancela para crear una nueva.
            </p>
          )}
          {editId === null && !canCreateMore && (
            <p className='mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700'>
              Limite alcanzado: maximo {MAX_MAIN_OFFERS} tarjetas principales. Elimina una para volver a crear.
            </p>
          )}

          {editId === null && canCreateMore ? (
            <MainOfferForm
              sectionTitle='Crear tarjeta'
              value={form}
              onChange={setForm}
