...
const staticFallback = [
  { title: "Madrid", description: "Provincia destacada" },
  { title: "Malaga", description: "Provincia destacada" },
  { title: "Cataluna", description: "Provincia destacada" },
  { title: "Cantabria", description: "Provincia destacada" },
];
...
type MainProvinceFormState = typeof emptyForm;

function validateMainProvinceForm(value: MainProvinceFormState) {
...
function MainProvinceForm({
  ...
  sectionTitle?: string;
  value: MainProvinceFormState;
  submitLabel: string;
  submitting: boolean;
  submitDisabled?: boolean;
  error: string;
  onChange: (next: MainProvinceFormState) => void;
  onSubmit: () => void;
  onCancel?: () => void;
}) {
  return (
    <div className='flex flex-col gap-3'>
      ...
      {value.image.trim() && !validateMainProvinceForm({ ...value, title: "ok", order: "", link: "" }) && (
        <div className='overflow-hidden rounded-xl border border-gray-200 bg-white'>
          <div className='relative h-32 w-full'>
            ...
            <p className='absolute bottom-2 left-2 right-2 text-sm font-semibold text-white drop-shadow-lg'>
              {value.title.trim() || "Titulo de provincia"}
            </p>
          </div>
        </div>
      )}
...
function MainProvincesList({
  ...
  isLoading: boolean;
  hasDbData: boolean;
  items: MainProvinceItem[];
  draggingId: number | null;
  deletingId: number | null;
  onStartEdit: (item: MainProvinceItem) => void;
...
export default function MainProvincesAdmin() {
  const { data, isLoading, mutate } = useSWR<{ success: boolean; payload: MainProvinceItem[] }>(
    "/api/platform/victor/home-page/main-provinces",
    fetcher
  );

  const items: MainProvinceItem[] = useMemo(() => {
    return data?.success ? (data.payload ?? []) : [];
  }, [data?.success, data?.payload]);
  const hasDbData = Boolean(!isLoading && data?.success && items.length > 0);
  const canCreateMore = items.length < MAX_PROVINCES;

  const [orderedItems, setOrderedItems] = useState<MainProvinceItem[]>([]);
  ...
  const handleCreate = async () => {
    if (!canCreateMore) {
      setCreateError(`Solo puedes tener ${MAX_PROVINCES} provincias principales.`);
      return;
    }

    const validationError = validateMainProvinceForm(form);
    if (validationError) {
    ...
    setCreateError("");
    try {
      const res = await fetch("/api/platform/victor/home-page/main-provinces", {
        method: "POST",
      ...
      if (!res.ok) {
        setCreateError(json.error ?? "Error al crear provincia");
        return;
      }
      setForm(emptyForm);
      mutate();
    } catch {
      setCreateError("Error al crear provincia");
    } finally {
      setCreating(false);
    }
  };

  const startEdit = (item: MainProvinceItem) => {
    setEditId(item.id);
    setEditError("");
    setCreateError("");
    setEditForm({
  ...
  const handleSaveEdit = async () => {
    const validationError = validateMainProvinceForm(editForm);
    if (validationError || editId === null) {
      setEditError(validationError || "No se pudo identificar la provincia");
      return;
    }
    ...
    setEditError("");
    try {
      const res = await fetch(`/api/platform/victor/home-page/main-provinces/${editId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id);
    try {
      await fetch(`/api/platform/victor/home-page/main-provinces/${id}`, { method: "DELETE" });
      mutate();
    } finally {
      setDeletingId(null);
    }
    ...
    try {
      const responses = await Promise.all(
        withRecalculatedOrder.map((item) =>
          fetch(`/api/platform/victor/home-page/main-provinces/${item.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              title: item.title,
    ...
    <details className='group mt-4 rounded-2xl border border-gray-200 bg-gray-50'>
      <summary className='list-none cursor-pointer p-4'>
        <div className='flex items-start justify-between gap-3'>
          <div>
            <h3 className='font-semibold text-(--main-arci)'>Provincias principales</h3>
            <p className='mt-1 text-xs text-gray-500'>Tarjetas de provincias destacadas en la seccion de home.</p>
          </div>
      ...
      <div className='px-4 pb-4'>
        <div className='rounded-2xl border border-gray-200 bg-white p-4'>
          <p className='mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400'>Orden de provincias</p>
        ...
        <div className='mt-4 rounded-2xl border border-gray-200 bg-white p-4'>
          <p className='mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400'>Formulario de provincia</p>
          {editId !== null && (
            <p className='mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700'>
              Editando provincia #{editId}. Guarda cambios o cancela para crear una nueva.
            </p>
          )}
          {editId === null && !canCreateMore && (
            <p className='mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700'>
              Limite alcanzado: maximo {MAX_PROVINCES} provincias principales. Elimina una para volver a crear.
            </p>
          )}

          {editId === null && canCreateMore ? (
            <MainProvinceForm
              sectionTitle='Crear provincia'
              value={form}
              onChange={setForm}
              onSubmit={handleCreate}
              submitting={creating}
              submitLabel='Agregar provincia'
              error={createError}
            />
          ) : editId !== null ? (
            <MainProvinceForm
              sectionTitle='Editar provincia'
              value={editForm}
