...
const staticFallback = [
  { title: "Cirugia", description: "Especialidad destacada" },
  { title: "Medico General", description: "Especialidad destacada" },
  { title: "Traumatologia", description: "Especialidad destacada" },
  { title: "Neurocirugia", description: "Especialidad destacada" },
];

const fetcher = (url: string) => fetch(url).then((r) => r.json());
const MAX_SPECIALITIES = 4;

const emptyForm = { title: "", image: "", link: "", order: "" };
type MainSpecialityFormState = typeof emptyForm;

function validateMainSpecialityForm(value: MainSpecialityFormState) {
  ...
  if (value.link.trim()) {
    try {
      const url = new URL(value.link.trim());
      if (!["http:", "https:"].includes(url.protocol)) {
...
function MainSpecialityForm({
  ...
  sectionTitle?: string;
  value: MainSpecialityFormState;
  submitLabel: string;
  submitting: boolean;
  submitDisabled?: boolean;
  error: string;
  onChange: (next: MainSpecialityFormState) => void;
  onSubmit: () => void;
  onCancel?: () => void;
}) {
  return (
    <div className='flex flex-col gap-3'>
      ...
      <label className='form-control'>
        <span className='label-text text-xs text-gray-500'>Enlace</span>
        <input
          className='input input-bordered input-sm w-full'
          value={value.link}
      ...
      {value.image.trim() && !validateMainSpecialityForm({ ...value, title: "ok", order: "", link: "" }) && (
        <div className='overflow-hidden rounded-xl border border-gray-200 bg-white'>
          <div className='relative h-32 w-full'>
            ...
            <p className='absolute bottom-2 left-2 right-2 text-sm font-semibold text-white drop-shadow-lg'>
              {value.title.trim() || "Titulo de especialidad"}
            </p>
          </div>
        </div>
      )}
      ...
      <div className='flex items-center gap-2 pt-1'>
        <button className='btn btn-sm text-white gap-1' style={{ backgroundColor: "var(--main-arci)" }} onClick={onSubmit} disabled={submitting || submitDisabled}>
...
function MainSpecialitiesList({
  ...
  isLoading: boolean;
  hasDbData: boolean;
  items: MainSpecialityItem[];
  draggingId: number | null;
  deletingId: number | null;
  onStartEdit: (item: MainSpecialityItem) => void;
          ...
          <div className='flex items-start justify-between gap-2'>
            <div className='min-w-0'>
              <div className='flex items-center gap-2'>
                ...
                <p className='font-semibold text-(--main-arci) truncate'>{item.speciality}</p>
              </div>
...
export default function MainSpecialitiesAdmin() {
  const { data, isLoading, mutate } = useSWR<{ success: boolean; payload: MainSpecialityItem[] }>(
    "/api/platform/victor/home-page/main-specialities",
    fetcher
  );

  const items: MainSpecialityItem[] = useMemo(() => {
    return data?.success ? (data.payload ?? []) : [];
  }, [data?.success, data?.payload]);
  const hasDbData = Boolean(!isLoading && data?.success && items.length > 0);
  const canCreateMore = items.length < MAX_SPECIALITIES;

  const [orderedItems, setOrderedItems] = useState<MainSpecialityItem[]>([]);
  ...
  const handleCreate = async () => {
    if (!canCreateMore) {
      setCreateError(`Solo puedes tener ${MAX_SPECIALITIES} especialidades principales.`);
      return;
    }

    const validationError = validateMainSpecialityForm(form);
    if (validationError) {
    ...
    setCreateError("");
    try {
      const res = await fetch("/api/platform/victor/home-page/main-specialities", {
        method: "POST",
      ...
      if (!res.ok) {
        setCreateError(json.error ?? "Error al crear especialidad");
        return;
      }
      setForm(emptyForm);
      mutate();
    } catch {
      setCreateError("Error al crear especialidad");
    } finally {
      setCreating(false);
    }
  };

  const startEdit = (item: MainSpecialityItem) => {
    setEditId(item.id);
    setEditError("");
    setCreateError("");
    setEditForm({
  ...
  const handleSaveEdit = async () => {
    const validationError = validateMainSpecialityForm(editForm);
    if (validationError || editId === null) {
      setEditError(validationError || "No se pudo identificar la especialidad");
      return;
    }
    ...
    setEditError("");
    try {
      const res = await fetch(`/api/platform/victor/home-page/main-specialities/${editId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id);
    try {
      await fetch(`/api/platform/victor/home-page/main-specialities/${id}`, { method: "DELETE" });
      mutate();
    } finally {
      setDeletingId(null);
    }
    ...
    try {
      const responses = await Promise.all(
        withRecalculatedOrder.map((item) =>
          fetch(`/api/platform/victor/home-page/main-specialities/${item.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              title: item.speciality,
    ...
    <details className='group mt-4 rounded-2xl border border-gray-200 bg-gray-50'>
      <summary className='list-none cursor-pointer p-4'>
        <div className='flex items-start justify-between gap-3'>
          <div>
            <h3 className='font-semibold text-(--main-arci)'>Especialidades principales</h3>
            <p className='mt-1 text-xs text-gray-500'>Tarjetas de especialidades destacadas en la seccion de home.</p>
          </div>
      ...
      <div className='px-4 pb-4'>
        <div className='rounded-2xl border border-gray-200 bg-white p-4'>
          <p className='mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400'>Orden de especialidades</p>
          <p className='mb-3 text-xs text-gray-500'>Arrastra y suelta para cambiar el orden de las tarjetas.</p>
        ...
        <div className='mt-4 rounded-2xl border border-gray-200 bg-white p-4'>
          <p className='mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400'>Formulario de especialidad</p>
          {editId !== null && (
            <p className='mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700'>
              Editando especialidad #{editId}. Guarda cambios o cancela para crear una nueva.
            </p>
          )}
          {editId === null && !canCreateMore && (
            <p className='mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700'>
              Limite alcanzado: maximo {MAX_SPECIALITIES} especialidades principales. Elimina una para volver a crear.
            </p>
          )}

          {editId === null && canCreateMore ? (
            <MainSpecialityForm
              sectionTitle='Crear especialidad'
              value={form}
              onChange={setForm}
              onSubmit={handleCreate}
              submitting={creating}
              submitLabel='Agregar especialidad'
              error={createError}
            />
          ) : editId !== null ? (
            <MainSpecialityForm
              sectionTitle='Editar especialidad'
              value={editForm}
