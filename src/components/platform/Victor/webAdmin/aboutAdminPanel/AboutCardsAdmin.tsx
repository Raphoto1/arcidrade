...
type AboutFormState = typeof emptyForm;
const MAX_ABOUT_CARDS = 9;

function validateAboutForm(value: AboutFormState) {
  ...
  if (value.description.trim().length > 500) return "La descripcion no puede superar 500 caracteres";
    ...
    if (!linkValue.startsWith("/")) {
      try {
        ...
        if (!["http:", "https:"].includes(url.protocol)) {
          return "El enlace debe ser ruta interna (/about) o URL http(s)";
        }
      } catch {
...
function AboutForm({
  ...
  submitting: boolean;
  error: string;
  onChange: (next: AboutFormState) => void;
  onSubmit: () => void;
  onCancel?: () => void;
}) {
  return (
    <div className='flex flex-col gap-3'>
      ...
      <label className='form-control'>
        ...
        <input
          ...
          placeholder='Ej: Identidad y proposito'
          maxLength={200}
        />
      </label>

      <label className='form-control'>
        <span className='label-text text-xs text-gray-500'>Descripcion *</span>
        <textarea
          className='textarea textarea-bordered textarea-sm w-full min-h-24'
          value={value.description}
          onChange={(e) => onChange({ ...value, description: e.target.value })}
          placeholder='Resumen breve de la card de About'
          maxLength={500}
        />
      </label>
      ...
      <label className='form-control'>
        <span className='label-text flex items-center gap-1 text-xs text-gray-500'>
          ...
          <span
            ...
            data-tip='Acepta rutas internas como /about y tambien URLs completas con http(s). Si lo dejas vacio, se guarda /about.'>
        ...
        <input
          ...
          placeholder='/about (por defecto)'
        />
      </label>

      {value.image.trim() && !validateAboutForm({ ...value, title: "ok", description: "ok", order: "", link: "" }) && (
        <div className='overflow-hidden rounded-xl border border-gray-200 bg-white'>
          <div className='relative h-32 w-full'>
            ...
            <p className='absolute bottom-2 left-2 right-2 text-sm font-semibold text-white drop-shadow-lg'>
              {value.title.trim() || "Titulo de card"}
            </p>
          </div>
        </div>
      )}
...
function AboutList({
  ...
  onStartEdit: (item: AboutItem) => void;
        ...
        <ul className='space-y-2'>
          {staticAboutItems.map((item: any) => (
            <div key={item.title} className='rounded-2xl border border-gray-100 bg-white p-3 opacity-70'>
              ...
              <p className='mt-1 text-xs leading-5 text-gray-500 line-clamp-2'>{item.description}</p>
            </div>
          ))}
        </ul>
      </div>
...
export default function AboutCardsAdmin() {
  const { data, isLoading, mutate } = useSWR<{ success: boolean; payload: AboutItem[] }>("/api/platform/victor/about-page", fetcher);

  const items: AboutItem[] = useMemo(() => {
    return data?.success ? (data.payload ?? []) : [];
  }, [data?.success, data?.payload]);
  const hasDbData = Boolean(!isLoading && data?.success && items.length > 0);
  const canCreateMore = items.length < MAX_ABOUT_CARDS;

  const [orderedItems, setOrderedItems] = useState<AboutItem[]>([]);
  ...
  const handleCreate = async () => {
    if (!canCreateMore) {
      setCreateError(`Solo puedes tener ${MAX_ABOUT_CARDS} cards de About.`);
      return;
    }
    ...
    setCreateError("");
    try {
      const res = await fetch("/api/platform/victor/about-page", {
        method: "POST",
  ...
  const startEdit = (item: AboutItem) => {
    setEditId(item.id);
    setEditError("");
    setCreateError("");
    setEditForm({
  ...
  const handleSaveEdit = async () => {
    const validationError = validateAboutForm(editForm);
    ...
    setEditError("");
    try {
      const res = await fetch(`/api/platform/victor/about-page/${editId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id);
    try {
      await fetch(`/api/platform/victor/about-page/${id}`, { method: "DELETE" });
      mutate();
    } finally {
      setDeletingId(null);
    }
    ...
    try {
      const responses = await Promise.all(
        withRecalculatedOrder.map((item) =>
          fetch(`/api/platform/victor/about-page/${item.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              title: item.title,
    ...
    <details className='group mt-4 rounded-2xl border border-gray-200 bg-gray-50' open>
      <summary className='list-none cursor-pointer p-4'>
        <div className='flex items-start justify-between gap-3'>
          <div>
            <h3 className='font-semibold text-(--main-arci)'>Cards principales de About</h3>
            <p className='mt-1 text-xs text-gray-500'>Administra los bloques institucionales de la pagina About.</p>
          </div>
        ...
        <div className='mt-4 rounded-2xl border border-gray-200 bg-white p-4'>
          ...
          {editId === null && !canCreateMore && (
            <p className='mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700'>
              Limite alcanzado: maximo {MAX_ABOUT_CARDS} cards de About. Elimina una para volver a crear.
            </p>
          )}
