...
export default function ProfesionalDetailFull(props: any) {
  ...
  const renderEmptyValue = () => (
    <span className='text-sm text-gray-400'>No se ha registrado este dato</span>
  );
  ...
  const {
    ...
    isEnabled: canDownloadProfessionalProfile(session?.user?.area),
  });

  const canDownloadProfile = canDownloadProfessionalProfile(session?.user?.area);
  ...
  return (
    <div className='Total grid gap-2 pt-2 overflow-x-hidden md:grid-cols-3'>
      <div className='bg-gray-200 p-4 rounded-sm z-10 flex flex-col gap-4'>
        <div className='flex flex-col items-center text-center'>
          ...
          <h1 className='mt-3 text-2xl fontArci text-center'>{`${personalData.name} ${personalData.last_name} `}</h1>
          <p className='text-center'>{mainStudy.title}</p>
          <div className='mt-2'>{renderHomologationBadge(mainStudy.isHomologated)}</div>
          {canDownloadProfile ? (
            <>
        ...
        <div className='bg-gray-50 rounded-sm p-3 shadow-xl'>
          <h1 className='text-2xl fontArci mb-2'>Presentación</h1>
          <div className='text-sm max-h-fit'>{renderValue(personalData.description)}</div>
      ...
      <div className='bg-gray-200 p-2 rounded-sm z-10 md:col-span-2 md:w-full'>
        ...
        <div className='dataSpace bg-gray-50 w-full rounded-sm p-2 grid mt-2 shadow-xl'>
          ...
          <div className='w-full md:text-lg'>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              ...
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(personalData.name)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Apellido:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(personalData.last_name)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='text-light'>Fecha de Nacimiento:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(personalData.birth_date ? fechaString : null)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Email:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(data?.payload.email)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Numero de Contacto:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(personalData.phone)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Pais:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(countryName?.name)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Ciudad:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(personalData.city)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Categoría de Profesión:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(mainStudy.sub_area ? useHandleCategoryName(mainStudy.sub_area) : null)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Profesión:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(mainStudy.title)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Institución:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(mainStudy.institution)}</p>
            </div>
            <div className='grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-2'>
              <h3 className='font-light'>Status:</h3>
              <p className='min-w-0 wrap-break-word text-(--main-arci) text-end'>{renderValue(mainStudy.status ? handleStatusName(mainStudy.status) : null)}</p>
            </div>
      ...
      <div className=' bg-gray-200 p-2 rounded-sm z-10 md:w-full md:text-lg'>
        ...
        <div className='flex flex-col gap-2'>
          {speciality.length > 0 ? speciality.map((item: any, index: number) => (
            <div key={index} className='bg-white rounded-md p-1 gap-2 flex flex-col'>
              <div className='flex items-start justify-between gap-2'>
                <h3 className='fontArci text-(--main-arci)'>{renderValue(item.title)}</h3>
                {renderHomologationBadge(item.isHomologated)}
              </div>
              <p className='text-sm text-(--soft-arci)'>{renderValue(item.institution)}</p>
              <p className='text-xs'>{renderValue(item.end_date ? handleDateToYear(item.end_date) : null)}</p>
              {item.link ? (
                <div className='flex justify-end'>
          ...
          )) : <p className='rounded-md bg-white p-3 text-sm text-gray-400'>No se ha registrado este dato</p>}
        </div>
      </div>
      <div className=' bg-gray-200 p-2 rounded-sm z-10 md:w-full md:text-lg'>
        <h1 className='text-2xl fontArci text mb-2'>Certificaciones</h1>
        <div className='flex flex-col gap-2'>
          {certifications.length > 0 ? certifications.map((item: any, index: number) => (
            <div key={index} className='bg-white rounded-md p-1 gap-2 flex flex-col'>
              <div className='flex items-start justify-between gap-2'>
                <h3 className='fontArci text-(--main-arci)'>{renderValue(item.title)}</h3>
                {renderHomologationBadge(item.isHomologated)}
              ...
              <div>
                ...
                <div className='line-clamp-3'>{renderValue(item.description)}</div>
                {item.description ? (
                  <ModalForPreviewTextLink title='Ver Más...'>
        ...
        <div className='flex flex-col gap-2'>
          {experience?.length ? experience.map((item: any, index: number) => (
            <div key={index} className='bg-white rounded-md p-1 flex flex-col gap-2'>
              ...
              <p className='text-sm text-(--soft-arci)'>{renderValue(item.institution)}</p>
              <div>
                <p className='text-xs'>{renderValue(item.city)}</p>
                <p className='text-xs text-(--soft-arci)'>
                  {renderValue(item.start_date || item.end_date ? `${handleDateToYear(item.start_date)} - ${handleDateToYear(item.end_date)}` : null)}
                </p>
              </div>
              <div>
                <h3 className='text-(--main-arci)'>Descripción:</h3>
                <div className='line-clamp-6'>{renderValue(item.description)}</div>
                {item.description ? (
                  <ModalForPreviewTextLink title='Ver Más...'>
                    <UserDescription description={item.description} />
                  </ModalForPreviewTextLink>
