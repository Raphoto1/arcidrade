...
import InstitutionCertificationUpdateForm from "@/components/forms/platform/institution/InstitutionCertificationUpdateForm";
import InstitutionConfirmDeleteCertificationForm from "@/components/forms/platform/institution/InstitutionConfirmDeleteCertificationForm";
import InstitutionFileCertificationForm from "@/components/forms/platform/institution/InstitutionFileCertificationForm";

export default function CertificateInstitution(props: any) {
  const date = new Date(props.year);
  const endDate = props.year ? date.getFullYear() : null;
  return (
    <div>
      <div className=' bg-gray-50 w-full rounded-sm p-2 grid grid-cols-2 gap-2 shadow-xl mt-2 justify-between items-center'>
        ...
        <div className='controles grid justify-center gap-2 mt-4'>
          <ModalForFormsRedBtn title='Eliminar Certificado'>
            <InstitutionConfirmDeleteCertificationForm id={props.id} />
          </ModalForFormsRedBtn>
          <ModalForFormsSoftBlue title='Agregar Respaldo'>
            <InstitutionFileCertificationForm id={props.id} />
          </ModalForFormsSoftBlue>
          <ModalForFormsSoftBlue title='Actualizar'>
            <InstitutionCertificationUpdateForm id={props.id} />
          </ModalForFormsSoftBlue>
        </div>
      </div>
    </div>
